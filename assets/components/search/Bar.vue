  ...
  <div class="max-w-3xl mx-auto mb-6 sm:mb-8">
    <div class="relative group">
      <div class="absolute inset-y-0 left-0 pl-3 sm:pl-4 md:pl-5 flex items-center pointer-events-none">
        <svg class="h-5 w-5 sm:h-6 sm:w-6 text-gray-400 group-focus-within:text-blue-600 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      ...
      <div class="absolute inset-y-0 right-0 pr-2 sm:pr-3 md:pr-4 flex items-center space-x-1 sm:space-x-2">
        <button
          v-if="modelValue"
          @click="$emit('clear')"
          class="p-3 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors touch-manipulation min-w-[48px] min-h-[48px] flex items-center justify-center"
          title="Clear search (Esc)"
          aria-label="Clear search"
        >
          <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
